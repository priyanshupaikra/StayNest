      ...
      <main className="flex-1 flex overflow-hidden">
        ...
        <div className="w-full lg:w-3/5 xl:w-1/2 overflow-y-auto px-4 md:px-6 py-6 border-r border-slate-200 dark:border-slate-700 bg-background-light dark:bg-background-dark">
          ...
          <div className="flex items-center justify-between mb-4">
            <h1 className="text-xl font-bold text-slate-900 dark:text-slate-100">{properties.length} places available</h1>
                ...
                <div className="flex flex-col flex-1 justify-between">
                  <div>
                    <div className="flex justify-between items-start">
                      <p className="text-sm text-slate-500">{prop.property_type} in {prop.city}</p>
                      <div className="flex items-center gap-1 text-sm font-semibold">
                        <span className="material-symbols-outlined text-sm text-primary" style={{ fontVariationSettings: "'FILL' 1" }}>star</span>
                        4.95
                      </div>
                    </div>
                    <Link to={`/property/${prop.id}`}><h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 mt-1">{prop.title}</h3></Link>
                    <div className="w-8 border-b-2 border-slate-200 dark:border-slate-700 mt-3 mb-2"></div>
                    <p className="text-sm text-slate-600 dark:text-slate-400">{prop.max_guests} guests · {prop.bedrooms} bedrooms · {prop.bathrooms} bathrooms</p>
                  </div>
